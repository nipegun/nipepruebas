...
#
# Ejecución remota (puede requerir permisos de root o sudo):
...
def fGenerarPrefijoBase(pMessage: Message) -> str:
  ...
  vPrefijoFecha = date_prefix(vTimestamp)
...
async def process_messages(client: TelegramClient, cfg: Config) -> tuple[int, int, int]:
  ...
  with Progress(
    ...
    async for message in client.iter_messages("me", reverse=True, limit=cfg.limit):
      total_messages += 1
      vBasePrefix = fGenerarPrefijoBase(message)
      ...
      if message.media:
        ...
        target_name = f"{vBasePrefix}-{guessed_name}"
      ...
      if (message.message or "").strip():
        write_text_file(message, vBasePrefix, cfg.output_dir)
        text_saved += 1
